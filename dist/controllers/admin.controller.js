...
exports.updateOrderStatus = exports.getAllOrders = exports.deleteProduct = exports.updateProduct = exports.createProduct = exports.deleteUser = exports.updateUser = exports.createUser = exports.getAllUsers = exports.getDashboardStats = void 0;
...
const getAllUsers = async (req, res) => {
    ...
    `;
        ...
        const countResult = await config_1.default.query(countQuery, countParams);
...
const createUser = async (req, res) => {
    ...
    try {
        ...
        const result = await config_1.default.query("INSERT INTO users (id, name, email, password, phone, role) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, name, email, phone, role, created_at", [userId, name, email, hashedPassword, phone, role]);
...
const updateUser = async (req, res) => {
    ...
    try {
        const result = await config_1.default.query("UPDATE users SET name = $1, email = $2, phone = $3, role = $4 WHERE id = $5 RETURNING id, name, email, phone, role, created_at", [name, email, phone, role, id]);
...
const deleteUser = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await config_1.default.query("DELETE FROM users WHERE id = $1 RETURNING id", [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: "User not found" });
        }
        res.status(200).json({
...
const createProduct = async (req, res) => {
    ...
    try {
        ...
        try {
         ...
         RETURNING *`, [productId, name, description, price, stock_quantity, image_url, category_id]);
            const product = productResult.rows[0];
            // Add product options (colors)
            if (colors && colors.length > 0) {
                for (const color of colors) {
                    await client.query("INSERT INTO product_options (product_id, option_type, option_value) VALUES ($1, $2, $3)", [productId, "color", color]);
                }
            }
            // Add product options (sizes)
            if (sizes && sizes.length > 0) {
                for (const size of sizes) {
                    await client.query("INSERT INTO product_options (product_id, option_type, option_value) VALUES ($1, $2, $3)", [productId, "size", size]);
                }
            }
...
const updateProduct = async (req, res) => {
    ...
    try {
        ...
        try {
         ...
         WHERE id = $7
         RETURNING *`, [name, description, price, stock_quantity, image_url, category_id, id]);
            ...
            if (colors && colors.length > 0) {
                for (const color of colors) {
                    await client.query("INSERT INTO product_options (product_id, option_type, option_value) VALUES ($1, $2, $3)", [id, "color", color]);
                }
            }
            // Add product options (sizes)
            if (sizes && sizes.length > 0) {
                for (const size of sizes) {
                    await client.query("INSERT INTO product_options (product_id, option_type, option_value) VALUES ($1, $2, $3)", [id, "size", size]);
                }
            }
            await client.query("COMMIT");
            // Format response
...
const deleteProduct = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await config_1.default.query("DELETE FROM products WHERE id = $1 RETURNING id", [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ message: "Product not found" });
        }
        res.status(200).json({
...
const updateOrderStatus = async (req, res) => {
    ...
    try {
        const result = await config_1.default.query("UPDATE orders SET status = $1 WHERE id = $2 RETURNING *", [status, id]);
        if (result.rows.length === 0) {
